...
exports.create_incidencia = async (req, res) => {
    ...
    let dataToUpdtae = ''
    for(let i = 0; i < refacciones.length; i++) {
        dataToUpdtae += `UPDATE refaccion set costo=${refacciones[i].costo}, venta=${refacciones[i].precioVenta} where idrefaccion=${refacciones[i].refaccion};`
        
    }

    const response3 = await pool.query(dataToUpdtae)
    res
    .status(201)
    .json({
        status: "success", 
