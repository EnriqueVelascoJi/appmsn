...
exports.create_incidencia = async (req, res) => {
    ...
    let dataToUpdtae = []
    for(let i = 0; i < refacciones.length; i++) {
        const query= `UPDATE refaccion set proveedor=${refacciones[i].proveedor}, costo=${refacciones[i].costo}, venta=${refacciones[i].precioVenta} WHERE idrefaccion=${refacciones[i].refaccion};`
