...
exports.create_incidencia = async (req, res) => {
    ...
    let dataToUpdtae = []
    for(let i = 0; i < refacciones.length; i++) {
        const query= `UPDATE refaccion set costo=${refacciones[i].costo}, proveedor=${refacciones[i].proveedor}, venta=${refacciones[i].precioVenta} WHERE idrefaccion=${refacciones[i].refaccion};`
